...
if mitigation_rows:
    if user_entered_mitigation_price:
        ...
        st.metric("Sub-Mitigation (No Penalty)", f"{total_substitution_cost:,.2f} Eur" if total_substitution_cost is not None)
    else:
...
total_with_pooling = total_cost + (pooling_cost_usd * exchange_rate if pooling_price_usd_per_tonne > 0 else 0)
...
if st.button("Export to PDF"):
    if not rows:
        ...
        if show_pooling_option and pooling_price_usd_per_tonne > 0:
            pdf.set_font("Arial", size=10)
            pooling_line = (f"CO2 Deficit Offset: {deficit_tonnes:,.2f} tCO2eq @ "
