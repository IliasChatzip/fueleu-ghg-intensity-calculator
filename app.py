import streamlit as st
import pandas as pd
import matplotlib.pyplot as plt

# === CONFIGURATION ===
BASE_TARGET = 91.16  # reference target for 2020
REDUCTIONS = {2025: 0.02, 2030: 0.06, 2035: 0.14, 2050: 0.80}
PENALTY_RATE = 0.64  # EUR per tonne CO2eq

# Global Warming Potentials
GWP = {"AR4": {"CH4": 25, "N2O": 298}, "AR5": {"CH4": 29.8, "N2O": 273}}

# Fuel database: name, LCV (MJ/g), WtT EF (gCO2eq/MJ), override EF for top fuels
fuels = [
    {"name": "Heavy Fuel Oil (HFO)", "lcv": 0.0405, "wtt": 13.5, "override_ef": 91.7368},
    {"name": "Marine Gas Oil (MGO)", "lcv": 0.0427, "wtt": 14.4, "override_ef": 90.7778},
    {"name": "Very Low Sulphur Fuel Oil (VLSFO)", "lcv": 0.0410, "wtt": 13.5, "override_ef": 91.3928},
    {"name": "Liquefied Natural Gas (LNG)", "lcv": 0.0500, "wtt": 79.0},
    {"name": "Liquefied Petroleum Gas (LPG)", "lcv": 0.0460, "wtt": 76.0},
    {"name": "Methanol (Fossil)", "lcv": 0.0199, "wtt": 94.0},
    {"name": "Biodiesel (UCO)", "lcv": 0.03727, "wtt": 14.9},
    {"name": "Biodiesel (Animal Fats)", "lcv": 0.03727, "wtt": 34.4},
    {"name": "Biodiesel (Sunflower Oil)", "lcv": 0.03727, "wtt": 45.3},
    {"name": "Biodiesel (Soybean Oil)", "lcv": 0.03727, "wtt": 56.0},
    {"name": "Biodiesel (Palm Oil)", "lcv": 0.03727, "wtt": 59.3},
    {"name": "HVO", "lcv": 0.0440, "wtt": 22.0},
    {"name": "Bio-LNG", "lcv": 0.0500, "wtt": 10.0},
    {"name": "Bio-Methanol", "lcv": 0.0199, "wtt": 11.0},
    {"name": "Green Hydrogen", "lcv": 0.1200, "wtt": 0.0},
    {"name": "Green Ammonia", "lcv": 0.0186, "wtt": 0.0},
    {"name": "E-Methanol", "lcv": 0.0199, "wtt": 1.0},
    {"name": "E-LNG", "lcv": 0.0500, "wtt": 1.0},
    {"name": "Onshore Power Supply (OPS)", "lcv": 1.0, "wtt": 0.0},
    {"name": "Synthetic Fuels", "lcv": 0.0420, "wtt": 10.0},
    {"name": "Recycled Carbon Fuels (RCFs)", "lcv": 0.0420, "wtt": 70.0},
]

# Compute target intensity for a given year
def target_intensity(year):
    if year <= 2020:
        return BASE_TARGET
    if year < 2025:
        return BASE_TARGET
    if year <= 2029:
        return BASE_TARGET * (1 - REDUCTIONS[2025])
    if year <= 2034:
        return BASE_TARGET * (1 - REDUCTIONS[2030])
    if year == 2035:
        return BASE_TARGET * (1 - REDUCTIONS[2035])
    # Linear interpolation to 2050
    y0, r0 = 2035, REDUCTIONS[2035]
    y1, r1 = 2050, REDUCTIONS[2050]
    frac = (year - y0) / (y1 - y0)
    red = r0 + frac * (r1 - r0)
    return BASE_TARGET * (1 - red)

st.title("FuelEU Maritime GHG Intensity Calculator")

# === SIDEBAR INPUTS ===
st.sidebar.header("Fuel Inputs")
selected = []
for i in range(1,6):
    opt = st.sidebar.selectbox(f"Fuel {i}", ["None"] + [f["name"] for f in fuels], key=f"f{i}")
    if opt != "None":
        mt = st.sidebar.number_input(f"{opt} mass (MT)", 0.0, 1e6, 0.0, step=100.0, key=f"m{i}")
        selected.append((opt, mt))

# Other options
year = st.sidebar.selectbox("Compliance Year", list(range(2025,2051)), index=0)
targ = target_intensity(year)
st.sidebar.markdown(f"**Target for {year}: {targ:.5f} gCO2eq/MJ**")
ops = st.sidebar.selectbox("OPS Reduction (%)", [0,1,2,3], index=0)
wind = st.sidebar.selectbox("Wind-Assisted Reduction (%)", [0,2,4,5], index=0)

# === CALCULATION ===
totE = 0.0
 totEm = 0.0
rows = []
for name, mt in selected:
    data = next(f for f in fuels if f['name']==name)
    mg = mt * 1e6
    E = mg * data['lcv']
    ef = data.get('override_ef', (3.114 + 0.00005*GWP['AR4']['CH4'] + 0.00018*GWP['AR4']['N2O'])/data['lcv'] + data['wtt'])
    ef *= (1 - ops/100)*(1 - wind/100)
    Em = E * ef
    totE += E
    totEm += Em
    rows.append({"Fuel":name, "Mass":mt, "Energy":E, "EF":ef, "Emissions":Em})

# === RESULTS ===
st.subheader("Fuel Breakdown")
st.dataframe(pd.DataFrame(rows))
if totE>0:
    gi = totEm/totE
    bal = totE*(targ - gi)
    pen = 0.0 if bal>=0 else abs(bal)/1000*PENALTY_RATE
    st.subheader("Summary")
    st.metric("GHG Intensity", f"{gi:.5f} gCO2eq/MJ")
    st.metric("Target Intensity", f"{targ:.5f} gCO2eq/MJ")
    st.metric("Compliance Balance", f"{bal:,.0f} gCO2eq")
    st.metric("Penalty (€)", f"{pen:,.2f}")

    # Forecast chart
    yrs = [2020]+list(range(2025,2051,5))
    vals = [target_intensity(y) for y in yrs]
    fig, ax = plt.subplots(figsize=(8,4))
    ax.plot(yrs, vals, linestyle='--', marker='o')
    ax.fill_between(yrs, vals, alpha=0.2)
    for x,y in zip(yrs,vals): ax.annotate(f"{y:.2f}", (x,y), textcoords="offset points", xytext=(0,5), ha='center')
    ax.grid(True)
    ax.set_title("GHG Target Forecast (2020–2050)")
    ax.set_xlabel("Year")
    ax.set_ylabel("gCO2eq/MJ")
    plt.tight_layout()
    st.pyplot(fig)
