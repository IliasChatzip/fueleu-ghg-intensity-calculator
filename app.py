...
def get_target_ghg_intensity(year):
    ...
    if year == 2035:
        return base * 0.86
    # Linear to 80% reduction by 2050
...
selected = []
for i in range(1,6):
    opt = st.sidebar.selectbox(f"Fuel {i}", ["None"] + [f["name"] for f in fuels], index=0, key=f"fuel_{i}")
    if opt != "None":
        mt = st.sidebar.number_input(f"{opt} mass (MT)", min_value=0.0, value=0.0, step=100.0, key=f"mt_{i}")
        if mt>0:
            fd = next(f for f in fuels if f["name"]==opt)
            selected.append({"name":opt, "lcv":fd["lcv"], "wtt":fd["wtt"], "mt":mt})
